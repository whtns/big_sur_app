...
techniques that are relevant to scRNA-seq analysis in BigSuR. It will grow over time as we uncover and try to address new issues!
...
- The interface may have frozen due to a bug in the code (often happens if an algorithm fails to converge) - you'll have to refresh your tab and start over from the beginning for now! Please let us know what happened just before the crash so we can look into what went wrong and how to prevent that error in the future.

Data upload fails completely
****************************

- We have found that when uploading ``h5ad`` files to BigSuR, Google Chrome tends to throw a security-related error. The root cause of this is not known to us yet, but we know that Mozilla Firefox seems not to have this issue. As such, we recommend users interact with BigSuR through Firefox for the time being.